...
var abbrs = {
    ...
    $("#milesSlide.slideRageBar span strong").text((milesValue*0.000621371).toFixed(0)+" Mi");
    if((minutesValue-4) > milesValue*0.000621371) {
        $('#minutesSlide').addClass('myactive');
        $('#milesSlide').removeClass('myactive');
    }else if((minutesValue-4) < milesValue*0.000621371) {
    ...
    function milesSlide (rad) {
        $( "#milesSlide" ).slider({
            value: rad.value*1609.34
        });
        circle.setRadius(rad.value);
        milesValue = rad.value;
        $('#milesSlide.slideRageBar span strong').text((milesValue*0.000621371).toFixed(0) + ' MI');
        /*if(e.value < 10)
        ...
        updateMilesRadius(rad.value);
    }

    function minutesSlide (min) {
        minutesValue = min.value;
        $("#minutesSlide.slideRageBar span strong").text(minutesValue +" Min");
        /*if(e.value < 10)
    ...
    function updateMilesRadius(rad){
        //milesValue = rad*0.000621371;
        ...
        customers.forEach(function(item, i){
            if(item.destinationDistance > milesValue*0.000621371){
    ...
    function getCustomerInfo(lat, lng, miles, min, index, timeline, callback){
        ...
        var request1 = $.ajax({
            ...
            data: JSON.stringify({"serviceId":serviceId,"latitude":lat, "longitude":lng,"miles": miles,"minutes":min, "userId":"", "subdomain":subDomains[index], "timeline": timeline}),
            contentType: "application/json; charset=UTF-8"
        });
        request1.success(function(result) {
